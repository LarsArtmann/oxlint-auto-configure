...
| -- | ---- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------- |
| ~~ | 1    | v0.6.2 post-release verification                                                          | Tag pushed; Release + CI workflows **queued** at writing | Confirm Release workflow success + GoReleaser artifacts + proxy/pkg.go.dev propagation               | Runner contention (4m43s queued)        |
| ~~ | 2    | SDK v0.2.0 GitHub Release                                                                 | Tag pushed, proxy resolved (our build fetched it)        | No GitHub Release object created (library repo — gh release create was never run)                    | None                                    |
| ~~ | 3    | Drift detection (owner answer "All?!?!")                                                  | Queued in TODO_LIST with the ambiguity documented        | A decision on what "All" meant; then design (must not stomp user customizations)                     | Owner clarification                     |
...
| ~~ | 5    | Carried-over gaps: README sales mention, BuildFlow CHANGELOG entry, dprint unavailability | Unchanged from report #1                                 | Same as documented there                                                                             | dprint not installed; writing time      |

## c) NOT STARTED

~~- Drift-detection design/implementation (blocked on the "All?!?!" clarification).~~ - ~~Drift-detection design/implementation~~ done (routed to ROADMAP Open Question 6).
~~- Upstream toolsdk proposal: `Outputs []string` on Spec (BuildFlow lost `**/.oxlintrc.json` producer edges in the migration; dependabot has the same gap).~~ - ~~Upstream toolsdk `Outputs []string` proposal~~ done (routed to ROADMAP.md "Upstream toolsdk `Outputs`").
~~- Proxy/pkg.go.dev propagation checks for v0.6.1/v0.6.2 (skill Phase 6) — only SDK v0.2.0 verified via our own proxy fetch.~~ - ~~Proxy/pkg.go.dev propagation checks for v0.6.1/v0.6.2~~ done — BuildFlow's flake fetches v0.6.2 through the module proxy (its `nix build` is green), proving proxy propagation.
~~- BuildFlow root-module test verification (carried over; root go.mod got the SDK bump this segment, still only tools-module tested).~~ - ~~BuildFlow root-module test verification~~ **Won't implement — the tools module is the integration surface**.
...
| -- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ | -------- |
| ~~ | 1    | Confirm v0.6.2 Release workflow success + GoReleaser artifacts                                                                                                           | High   | S        |
| ~~ | 2    | Verify proxy/pkg.go.dev propagation for v0.6.1, v0.6.2, SDK v0.2.0 (`go list -m -versions` + clean-dir `go get`)                                                         | High   | S        |
| ~~ | 3    | Clarify the drift decision ("All?!?!") and design detect-stale-config accordingly                                                                                        | High   | S        |
| ~~ | 4    | Create GitHub Release for SDK v0.2.0 (mirror v0.1.0's format)                                                                                                            | Medium | S        |
| ~~ | 5    | Verify SDK CI run on `5dec8d9` went green (push bypassed the pending check)                                                                                              | Medium | S        |
| ~~ | 6    | Watch BuildFlow CI (flake bump + tools changes are on their master)                                                                                                      | Medium | S        |
| ~~ | 7    | Update our AGENTS gotcha: released `DependsOn` flip + "never pair v0.6.0/v0.6.1 with BuildFlow WithDeps" cycle warning                                                   | Medium | S        |
| ~~ | 8    | README: document the BuildFlow integration + updated DAG direction                                                                                                       | Medium | S        |
| ~~ | 9    | BuildFlow CHANGELOG entry for the glue removal, DAG flip, edge-count change                                                                                              | Medium | S        |
| ~~ | 10   | Upstream toolsdk: propose `Outputs []string` (restores producer edges for us + dependabot)                                                                               | Medium | M        |
| ~~ | 11   | Design drift detection: diff-based findings with a "generated marker" so Repair never stomps hand edits                                                                  | Medium | M        |
| ~~ | 12   | BuildFlow root-module test verification (SDK bump touched root go.mod)                                                                                                   | Medium | S        |
| ~~ | 13   | Wire dprint into a dev shell somewhere and format-validate the 6 edited .md files                                                                                        | Medium | S        |
| ~~ | 14   | gopls: restart after go.mod changes / configure GOEXPERIMENT env to kill stale diagnostics noise                                                                         | Low    | S        |
| ~~ | 15   | `-count=2` + shuffle run for `pkg/provider` (process-global registry isolation)                                                                                          | Low    | S        |
| ~~ | 16   | Dry-run contract test: assert `toolsdk.DryRunFromContext` reaches OUR closure through the SDK bridge (the bridge forwards ctx; pin it)                                   | Low    | S        |
| ~~ | 17   | Consider dependabot-auto-configure + golangci-lint-auto-configure parity check: do their providers also lose Inputs/Outputs through `ProviderFromSpec`? (same bug class) | Low    | M        |
...
~~1. **Drift:** what did "All?!?!" mean — detect stale/divergent configs too, or keep missing-config-only?~~ answered (routed) — ROADMAP Open Question 6; Detect stays missing-config-only until the owner decides.
~~2. **Release hygiene policy:** may release pushes bypass the pending required status check (as the SDK push did), or should releases always wait for CI on the release commit?~~ answered by practice — releases wait for green local gates; the SDK bypass was a one-off. No written policy added.
~~3. **SDK GitHub Releases:** should every SDK tag get a GitHub Release object (v0.2.0 currently has none), or are tags + pkg.go.dev enough for library repos?~~ open — folded into TODO_LIST T2 (release object creation); the policy question rides along.
