...
4. **Markdown or HTML for status reports?** → **Resolved 2026-09-22: Markdown canonical — "always just markdown unless I ask for HTML" (owner).** HTML is produced only on explicit request.
...
8. **Cut v0.7.0 now or batch?** → **Resolved 2026-09-22: tag now** (owner). The pre-release gate passed end-to-end (module consistency, build, vet, race tests, lint, `goreleaser check`, full snapshot), and the v0.6.4 release-pipeline blocker (Docker SBOM attestation vs. runner driver) is fixed in the same change. **Outcome:** v0.7.0 hit a second release blocker (the `dockers_v2` context layout — binaries at `$TARGETPLATFORM/<binary>`, not the root) and stayed tag-only like v0.6.4; **v0.7.1 shipped the same night with the full artifact set** (archives, deb/rpm/apk, cosign signatures, SBOMs, Docker images with attestations) and is the release to consume.
