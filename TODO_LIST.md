...
| C4 | Pin `anchore/sbom-action/download-syft@v0` to a full SHA (the only floating action left) and drop the unused `HOMEBREW_TAP_GITHUB_TOKEN` env | `.github/workflows/release.yml:33,49`                | S      |
...
| T2 | Publish SDK v0.2.0 GitHub Release object (tag exists; `gh release create` never ran in the SDK repo) and trigger pkg.go.dev fetch for the latest tag | `docs/status/2026-09-11_14-49_*.md` §b.2  | S      |
