...
```bash
nix build .                    # Build binary (fast: enableCheck=false, tests skip)
...
- **Systems** — go-standard's built-in defaultSystems (x86_64-linux, aarch64-linux, aarch64-darwin; x86_64-darwin dropped by nixpkgs 26.11); no `systems` flake input needed
- **No GOEXPERIMENT needed** — `encoding/json/v2` is default-on since Go 1.27; the flake sets no experiment env anywhere (removed 2026-09-22; full suite verified without the flag)
- **Checks** — `format`/`treefmt` (hermetic treefmt: matching go on PATH + `GOTOOLCHAIN=local`), `build`, and `test` (doCheck=true + oxlint in `nativeCheckInputs`; reuses the package so goModules are shared)
- **Formatter** — `nixfmt` via `formatter` output

### Testing

```bash
GOWORK=off GOTOOLCHAIN=auto go test -race ./...   # Run tests with -race
GOWORK=off GOTOOLCHAIN=auto go test -cover ./...  # Coverage report
GOWORK=off GOTOOLCHAIN=auto go vet ./...          # Run go vet
nix flake check .                                    # All checks via nix
```

- **`GOEXPERIMENT=jsonv2` no longer required** — `encoding/json/v2` was experiment-gated through Go 1.26 but is default-on in Go 1.27+ (go.mod floor is `go 1.27`); verified 2026-09-22 that build, vet, and the full test suite pass without the flag. The flake no longer sets it (stale references in CI/GoReleaser env are harmless).
...
- `github.com/larsartmann/go-finding` v1.13.0 (pipeline v1.10.0) — Unified static analysis model (public; branded types `RuleName`/`ToolName`/`ID`/`FilePath` in `NewFinding`) + `go-finding/pipeline` submodule
- `github.com/larsartmann/go-finding/toolsdk` v1.13.0 — BuildFlow provider contract (`Spec`/`Register`/`All`, `Trigger.NotRequires`); used by `pkg/provider` (sub-module tag `toolsdk/v1.13.0`; the flake input pins the repo at root tag `v1.13.0`, which contains the toolsdk changes — resolves via module proxy in nix like `pipeline`)
...
- `github.com/larsartmann/go-error-family` v0.10.1 — Structured error family helpers (transitive dep of `go-atomic-write` v0.5.1)
...
Then bump `pkg/rule/rules_version.txt` to the generating oxlint version and update the counts (`TestRegistryTotal` in `pkg/rule/registry_test.go`, the two count assertions in `internal/cli/commands_test.go`, `TestEmbeddedVersion`) plus the README rule-statistics table. CI installs oxlint from `rules_version.txt` and hard-fails on drift between the installed binary and the embedded version — a refresh is only complete when CI's drift check passes. Registry state: **870 rules @ 1.82.0**, 111 enabled by default (updated 2026-09-22).
...
- **GOEXPERIMENT=jsonv2 is obsolete** — `encoding/json/v2` was build-tag gated through Go 1.26 and is default-on in Go 1.27+ (go.mod floor is `go 1.27`). No direct `go` invocation needs the flag anymore; the flake stopped setting it on 2026-09-22 (was an `extraBuildAttrs.preBuild` export + `shellExtraEnv`).
...
- **Accepted art-dupl baseline (at `-t 1`)** — clone groups that are deliberate, do not "fix" them: (1) `t.Parallel()` + `reg := testregistry.Load(t)` test boilerplate (`paralleltest` requires direct statements); (2) the `~string`-to-`[]string` conversion loops in `detect.FormatTypes` and `profile.AllProfileNames` (stdlib has no `slices.Map`; a shared generic helper would over-couple — rationale comments inline); (3) the `ToJSON` vs `marshalConfigJSON` layered error wraps (different layers, different error context). The `var zero ExternalPlugin` returns were deduplicated into `rule.findExternalPlugin` (2026-09-22). If art-dupl reports beyond these, judge individually.
...
- **CI security** — GitHub Actions runs four jobs on every push/PR (plus `workflow_dispatch`): test (with `go mod tidy` consistency check; oxlint installed pinned to the embedded rules version — ubuntu-latest runners no longer ship pnpm), security (govulncheck pinned to `v1.8.0`), lint (golangci-lint pinned to `v2.13.2` — MUST match the `.golangci.yml` linter generation: the config enables `exhaustruct_v5`/`wsl_v5`, which 2.12.2's `config verify` rejects outright), nix (`nix flake check` — the `--systems` flag does not exist and fails the job). During the Jul–Sep 2026 billing block the CI workflow was left `disabled_manually`; after the billing fix it must be re-enabled via `gh workflow enable ci.yml` or pushes silently skip CI (Release stayed active, masking this). A fifth scheduled workflow, `workflow-health.yml`, canaries exactly that failure mode: it asserts every workflow in `.github/workflows/` has state `active` (weekly cron + `workflow_dispatch`).
- **Nix toolchain vs go.mod floor** — nixpkgs' default go (1.26.7) lags the repo's `go.mod` floor (`go 1.27`); the flake selects `goPkgAttr = "go_1_27"` (nixpkgs go_1_27 = 1.27.1, binary-cached — no source build, no toolchain downloads; also feeds the hermetic treefmt check and `apps.test`). History: 2026-09-19→22 the sandboxed treefmt/goimports check died on a network toolchain download (no DNS in sandbox); fixed first via go-nix-helpers rev `29e39b2…` + `goTarballVersion`/`goTarballHash` building go from the go.dev tarball, then simplified 2026-09-22 once nixpkgs shipped go_1_27. When the floor moves past nixpkgs again, bump `goPkgAttr`, or temporarily set `goTarballVersion`+`goTarballHash` (`nix store prefetch-file --hash-type sha256 https://go.dev/dl/goX.Y.Z.src.tar.gz`).
- **Release pipeline** — Tag-push (`v*`) triggers GoReleaser v2: cosign-signed artifacts, SBOMs, checksums, deb/rpm/apk, ghcr.io Docker images (login-action with `GITHUB_TOKEN`), GitHub Release. The GoReleaser step sets `GOEXPERIMENT=jsonv2` — without it the build fails (`encoding/json/v2` is build-tag gated). Docker SBOM attestations (`dockers_v2.sbom: true`) require a containerd-capable buildx driver: `docker/setup-buildx-action` runs before GoReleaser — without it the runner's default `docker` driver rejects `--attest=type=sbom` (`Attestation is not supported for the docker driver`) and the release aborts pre-publish (v0.6.4, 2026-09-22: tag exists, no GitHub Release; fold-forward, never re-tag). `dockers_v2` also changed the Docker build context: binaries live at `$TARGETPLATFORM/<binary>` in the context, so the Dockerfile needs `ARG TARGETPLATFORM` + `COPY $TARGETPLATFORM/<binary> ...` — a v1-style root `COPY` fails with `failed to calculate checksum ... not found` (v0.7.0, 2026-09-23, also tag-only). Verify Dockerfile changes locally with a real docker build: `goreleaser release --snapshot --clean --skip=sign,validate,sbom` (needs local docker; `--skip=sbom` because syft isn't installed locally). Homebrew/Scoop/Nix tap uploads are `skip_upload: true` (no usable tap repos — `nur-packages` doesn't exist); release notes advertise go install / flake run / binary downloads instead. Validate release changes locally with `GOEXPERIMENT=jsonv2 goreleaser release --snapshot --clean --skip=docker,sbom,sign,validate` — a local snapshot run catches brew/nix template errors that the CI runner masks (nix pipe silently skips there). July–Sept 2026 Actions failures were an account billing block (fixed 2026-09-11); the repo is now public, so Actions minutes are free. `goreleaser check` reports deprecation warnings (`brews`, `dockers`) that do not block releases.
...
- **BuildFlow integration** — `pkg/provider` self-registers a `toolsdk.Spec` at import time; BuildFlow consumes it via a blank import of `github.com/larsartmann/oxlint-auto-configure/pkg/provider` in `tools/providers/sdk_imports.go`. Spec name `oxlint-auto-configure` must stay in sync with BuildFlow's `config.ToolOxlintAutoConfigure`. Detect only flags a MISSING `.oxlintrc.json` (never config drift — flagging drift would lead Repair to stomp user customizations); Repair never overwrites an existing config. HealthCheck (`healthCheckDrift`) reports config drift ADVISORILY: BuildFlow treats a health-check failure as warn-log + summary entry only — it never skips the tool and never triggers a repair, so drift can be flagged without stomp risk (owner decision 2026-09-22: "use toolsdk to the max"). Registration is a package-level var (`toolsdk.Register` panics on invalid specs at startup). Version pairing gotcha: do NOT pair v0.6.0/v0.6.1 with BuildFlow's `WithDeps(config.ToolOxlintAutoConfigure)` — the DAG flip landed in v0.6.1, so pairing a pre-flip provider with a post-flip consumer creates a dependency cycle; use v0.6.2+.
- **Stale flake pins are NOT reliably caught by builds** — `go-finding` sat at v1.8.0 in flake.nix while go.mod required v1.10.0 (additive API kept it compiling). Recurred 2026-09-22, found by audit not by builds: `gogenfilter` pinned v3.4.0 vs go.mod v3.6.1 and `go-error-family` v0.10.0 vs v0.10.1 — API-compatible pins compile silently, so the nix binary can diverge from `go build` output. On every go.mod dep bump, diff the LarsArtmann requires against the flake input tags AND the `deps` map; verify with `nix build` + vendorHash refresh.
- **`json.Deterministic(true)` is REQUIRED for every output-facing `json.Marshal`** — `encoding/json/v2` serializes map keys in an order that CHANGES between calls on the same map (verified: 9 distinct outputs in 200 marshals of one map). Without the flag, `ToJSON` shuffles rules/settings/env keys per run: regenerated configs churn, VCS diffs go noisy, and `pkg/diff` `formatValue`-based comparisons flap (a fresh-repair-then-health-check comparison randomly reported drift). Pinned by `TestToJSONIsDeterministic`. Sites: `pkg/config/generator.go` (ToJSON), `pkg/diff/differ.go` (formatValue), `pkg/format/format.go`, `internal/cli/cmd_report.go`.
- **`recommended` profile is REMOVED; `strict` is the only name** — both produced byte-identical output (owner decision 2026-09-22). `profile.Parse` maps `"recommended"` to an error wrapping `profile.ErrProfileRemoved` that names `strict` as the replacement; unknown names wrap `profile.ErrInvalidProfile`. `config.ErrInvalidProfile` is a deprecated alias of the same value. CLI default (`cmd_root.go`) and the BuildFlow Repair pipeline (`provider.go`) both use `ProfileStrict`. Never reintroduce alias profiles for one behavior.
- **oxlint stdout is not clean JSON** — oxlint 1.82 signals startup failures (e.g. an unloadable `jsPlugins` package) with exit code 1 and plain text on stdout (stderr empty) — the same code it uses for findings, so `ExitError.Stderr` heuristics cannot catch it. It also prepends notices like "No files found to lint." before the JSON on stdout. `pkg/oxlint.decodeOutput` handles both: whole-output parse first, embedded `{...}` substring retry, structured `FindingError` with a stdout snippet on failure. `analyze` additionally fails the command on `result.PartialErrors` — under `GracefulDegradation` the pipeline collects detector errors silently, and ignoring them turns oxlint failures into a lying "no findings — project is clean".
- **GoReleaser v2 config (2026-09 state)** — migrated off deprecated keys: `brews` → `homebrew_casks` (note: `binaries` list, not the deprecated `binary`; casks use `Casks/` directory), `dockers` + `docker_manifests` → one multi-platform `dockers_v2` entry (images are full refs incl. registry, tags are bare; `sbom: true` builds an image SBOM; cosign signs manifests via `docker_signs` with `artifacts: manifests`). Local goreleaser is 2.17.1 (nix); validate config changes with `scripts/pre-release-check.sh` — it runs the full gate (tidy-consistency via temp-file comparison, build, vet, race tests, lint, `goreleaser check`, snapshot).
- **go.mod go-directive canonicalization** — with `GOTOOLCHAIN=auto`, `go mod tidy` under Go 1.27.1 rewrote `go 1.27` → `go 1.27.1` (full patch version recording). This is canonical now: keep the patch version in go.mod (it also makes toolchain resolution exact instead of floating to latest patch); a `git diff --exit-code` consistency check fails spuriously on a dirty worktree — CI is fine (clean checkout), local gates must compare pre/post-tidy snapshots instead.
