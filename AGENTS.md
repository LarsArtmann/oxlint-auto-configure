...
- `github.com/larsartmann/go-finding` v1.12.0 (pipeline v1.10.0) — Unified static analysis model (public; branded types `RuleName`/`ToolName`/`ID`/`FilePath` in `NewFinding`) + `go-finding/pipeline` submodule
...
- **Nix toolchain vs go.mod floor** — nixpkgs' go lags the repo's `go.mod` floor (go 1.27.1 vs nixpkgs 1.26.7), so the sandboxed treefmt/goimports check tried a network toolchain download and died (sandbox has no DNS; this broke the CI nix job 2026-09-19 → 09-22). Fixed by bumping the `go-nix-helpers` flake input to rev `29e39b2…` (hermetic treefmt check: matching go on PATH + `GOTOOLCHAIN=local`) and setting `goTarballVersion`/`goTarballHash` in the `go-standard` module config to build the exact floor toolchain from the go.dev tarball. When the go.mod floor moves again, bump `goTarballVersion`+`goTarballHash` with it (`nix store prefetch-file --hash-type sha256 https://go.dev/dl/goX.Y.Z.src.tar.gz` gives the SRI hash).
