...
- **CI security** — GitHub Actions runs four jobs on every push/PR (plus `workflow_dispatch`): test (with `go mod tidy` consistency check; oxlint installed pinned to the embedded rules version — ubuntu-latest runners no longer ship pnpm), security (govulncheck pinned to `v1.8.0`), lint (golangci-lint pinned to `v2.12.2`), nix (`nix flake check` — the `--systems` flag does not exist and fails the job). During the Jul–Sep 2026 billing block the CI workflow was left `disabled_manually`; after the billing fix it must be re-enabled via `gh workflow enable ci.yml` or pushes silently skip CI (Release stayed active, masking this). A fifth scheduled workflow, `workflow-health.yml`, canaries exactly that failure mode: it asserts every workflow in `.github/workflows/` has state `active` (weekly cron + `workflow_dispatch`).
