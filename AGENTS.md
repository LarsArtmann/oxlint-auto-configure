...
- **CI security** — GitHub Actions runs four jobs on every push/PR (plus `workflow_dispatch`): test (with `go mod tidy` consistency check; oxlint installed via `npm install -g oxlint` — ubuntu-latest runners no longer ship pnpm), security (govulncheck), lint (golangci-lint pinned to `v2.12.2`), nix (`nix flake check` — the `--systems` flag does not exist and fails the job). During the Jul–Sep 2026 billing block the CI workflow was left `disabled_manually`; after the billing fix it must be re-enabled via `gh workflow enable ci.yml` or pushes silently skip CI (Release stayed active, masking this).
