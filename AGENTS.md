...
- **Release pipeline** — Tag-push (`v*`) triggers GoReleaser v2: cosign-signed artifacts, SBOMs, checksums, deb/rpm/apk, ghcr.io Docker images, GitHub Release. Homebrew/Scoop/Nix tap uploads are `skip_upload: true` (no usable tap repos — `nur-packages` doesn't exist); release notes advertise go install / flake run / binary downloads instead. July–Sept 2026 Actions failures were an account billing block (fixed 2026-09-11); the repo is now public, so Actions minutes are free. `goreleaser check` reports deprecation warnings (`brews`, `dockers`) that do not block releases.
