...
| BuildFlow integration (toolsdk Spec provider)                | FULLY_FUNCTIONAL | `pkg/provider/provider.go`, `pkg/provider/provider_test.go`                              | Self-registers via `toolsdk.Register`: Detect = missing-config finding, Repair = generate recommended config (dry-run aware, never overwrites). BuildFlow consumes via blank import. |
